...
export default function ChatBot() {
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden">
      {/* Sidebar */}
      <div 
        className={`fixed md:relative top-0 left-0 h-full w-64 bg-white/20 backdrop-blur-xl shadow-lg transform transition-transform duration-300 ease-in-out border border-white/20 z-50 ${
      ...
      >
        <div className="p-4 border-b border-white/20">
          <div className="flex items-center justify-between">
            ...
            <button onClick={toggleSidebar} className="p-2 hover:bg-white/10 rounded-lg md:hidden">
              <X className="w-5 h-5" />
            </button>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col h-full overflow-hidden relative">
        ...
        <header className="fixed top-4 left-4 right-4 px-4 py-3 flex items-center justify-between bg-white/30 backdrop-blur-md rounded-2xl border border-white/30 shadow-sm z-40">
          <div className="flex items-center gap-2">
            <button onClick={toggleSidebar} className="p-2 hover:bg-white/20 rounded-lg mr-2 md:hidden">
        ...
        <main className="flex-1 overflow-y-auto pt-24 px-4">
          <div className="container mx-auto max-w-3xl">
            {!isConversationMode ? (
              <div className="flex-1 flex items-center justify-center flex-col min-h-[calc(100vh-12rem)]">
                <div className="text-center space-y-6 mb-8">
                  <h1 className="text-4xl md:text-5xl font-bold tracking-tight text-gray-800">
                    {welcomeMessage}
                  </h1>
                  <p className="text-gray-600 text-lg">Your Mental Therapist</p>
                </div>
